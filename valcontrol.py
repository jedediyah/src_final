...
class KeyboardTeleop(object):
    ...
    HEAD_BINDINGS = OrderedDict([
        ...
        ('c', {'joint_index': 1, 'min': -0.5, 'max': 0.7}),
        ('v', {'joint_index': 2, 'min': -1.0, 'max': 1.0}),
        ('g', {'joint_index': 'reset'}),
    ])

    NECK_BINDINGS = OrderedDict([
        ('b', {'joint_index': 0, 'min': 0.0, 'max': 0.7}),  # lowerNeckPitch? 'min': 0, 'max': 1.162
    ...
    def sendArmTo(self, side, joint_angles):    
        ...
        self._append_trajectory_point_1d(
            msg, 2.0, self.joint_values['%s arm' % side])

        self.arm_publisher.publish(msg)
        return 
        
    def step_left(self,distance):
        print "Stepping left ", distance 
        ...
        self.loginfo('Stepping left, sideways...\n')
        msg = self.getEmptyFootsetListMsg()
        msg.footstep_data_list.append(self.createTranslationFootStepOffset(
            FootstepDataRosMessage.LEFT, [0.0, distance, 0.0]))
        msg.footstep_data_list.append(self.createTranslationFootStepOffset(
            FootstepDataRosMessage.RIGHT, [0.0, distance, 0.0]))
        self.execute_footsteps(msg)
        self.loginfo('done stepping left\n')

    def step_right(self,distance):
        print "Stepping right ", distance 
        left_foot_world = self.tfBuffer.lookup_transform(
            'world', self.LEFT_FOOT_FRAME_NAME, rospy.Time())
        right_foot_world = self.tfBuffer.lookup_transform(
            'world', self.RIGHT_FOOT_FRAME_NAME, rospy.Time())
        ...
        self.loginfo('Stepping right, sideways...\n')
        msg = self.getEmptyFootsetListMsg()
        msg.footstep_data_list.append(self.createTranslationFootStepOffset(
            FootstepDataRosMessage.RIGHT, [0.0, -distance, 0.0]))
        msg.footstep_data_list.append(self.createTranslationFootStepOffset(
            FootstepDataRosMessage.LEFT, [0.0, -distance, 0.0]))        
        self.execute_footsteps(msg)
        self.loginfo('done stepping right\n')
